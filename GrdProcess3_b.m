...
%% Main
function GrdProcess3(cycle,theta,RowGap,GritGap,k_dev,shape,omega_orien,wheel_type,Rarea,batnum,sigmah,sigmasw)
warning off;
tic;
if isempty(cycle)
    cycle = 1;
end
%  wheel_type=1;   % 1-random; 2-tgw
cof_cal_mode=2; % 0-none; 1 -simplified mode; 2-accurate mode
%%%
% theta=80;   % line tilt angle:30 40 50 60 70 80
% SepGap=0.5; % column gap: 1,3,5,7
% RowGap=4;   % row gap: 1,3,5,7
% k_dev=1;    % position deviation para: 1,3,5,7
workpiece_length=1000;   %the length of a workpiece
workpiece_width=500;     %max(grits.posx);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% geometrical parameters
%  shape = 1; % 1-trapezoid&pyramid 2-shpere, 3-ellipsoid
trim_h = 0;
% omega_orien-number of edges for pyramid and trapezoid
% omega_orien-orientation for ellipsoid
%  omega_orien = 3; 
h2w_ratio = 1; % h2w_ratio = height/(2*R)
%  Rarea = 0; % Rarea-ratio of trapezoid area of top face to bottom face
% sigmah = 0.1;
% sigmasw = 0.1;
fillet_mode = 0; % fillet_mode:1-with fillet, 2 or 0-without fillet
geoparam = [shape,trim_h,omega_orien,h2w_ratio,Rarea,sigmah,sigmasw,fillet_mode];
%%
% if shape == 1
%     if Rarea<=1e-7
%         SP = 'pyramid';
...
%     SP = 'ellipsoid';
% end
%only for studying distribution here
% SP = 'Distribution';
if batnum == 1 
    SP = 'effectofshape';
elseif batnum == 2 
...
Rsigma=0;
%%%%%%%
[status, msg, msgID] = mkdir(SP);
[status, msg, msgID] = mkdir(['gprofile_temp\' SP]);
if wheel_type==1
    num_grits=2000;
    premise = [num_grits Rsigma geoparam];
    filename=[SP '\\' 'C' num2str(cycle) 'N' num2str(num_grits) 'Rsg' num2str(Rsigma) 'w' num2str(omega_orien) ...
         'Rarea' num2str(Rarea) 'hsg' num2str(sigmah) 'swsg' num2str(sigmasw) ] ; %'Rhw' num2str(h2w_ratio) 'FT' num2str(fillet_mode)
    [grit_profile_all]=bubbleSimulator(filename,num_grits,geoparam,Rsigma);
else
    num_grits=2400; %only recommend 500,1k and 2k, 2.4k
    premise = [num_grits,theta,Rsigma,GritGap,RowGap,k_dev,geoparam];
    filename=[SP '\\'  'C' num2str(cycle) '_tgw' num2str(theta) 'Rsg' num2str(Rsigma) 'kd' num2str(k_dev)...
        'Rgap' num2str(RowGap) 'Ggap' num2str(GritGap) 'w' num2str(omega_orien) ...
         'Rarea' num2str(Rarea)  'hsg' num2str(sigmah) 'swsg' num2str(sigmasw)] ; % 'Sgap' num2str(SepGap)'Rhw' num2str(h2w_ratio) 'FT' num2str(fillet_mode)
    [grit_profile_all]=TGW_generator(filename,theta,GritGap,RowGap,...
        k_dev,workpiece_length,workpiece_width,geoparam,Rsigma);
end

[Ra,C_grit,F_n_steadystage,F_t_steadystage,num_mode,percent_mode]=GrindingProcess(filename,grit_profile_all,cof_cal_mode,workpiece_length,workpiece_width);
writematrix([convertCharsToStrings(char(datetime)) cycle premise Ra C_grit F_n_steadystage F_t_steadystage num_mode percent_mode],['w' num2str(wheel_type) 'batch_' num2str(batnum) '_premise.csv'],'WriteMode','append');
toc;
end
...
for c_i=1:num_cycle
    for g_i=1:numgrits
        ...
        v_i=v_i+1;
        vgrit(v_i,1)=g_i;
        vgrit(v_i,2)=round(grits.posx(g_i));
...
%     area_chp(t_tick,v_i)=temp_chparea;
    ...
    %% calculation of microinteration
...
%     plot(1:num_vgrits,hmax(:,1),'k');title('uct vs grit #');
    ...
    F_n_total=sum(F_n,2);
    F_t_total=sum(F_t,2);
    F_n_steadystage=mean(F_n_total(45:55));
    F_t_steadystage=mean(F_t_total(45:55));
    %         figure;
    %         if size(u,1)==2501
    %             t_axis=0:dt:t_step;
    %         else
